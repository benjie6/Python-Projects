...
def describe_game(name):
    ...
    if it is new, get the urser's name.
...
def score(nice,mean,name):
    ...
    if nice > 2: #if condition is valid, call win function passing in the variables so it can use them
        win(nice,mean,name)
    if mean > 2: #id condition is valid, call lose function passing in the variables so it can use them
        lose(nice,mean,name)
    else:        #else, call nice_mean function passing in the variable ao it can use it
...
def lose(nice,mean,name):
    # substitute the {} wildcards with our variable values
    print("\nAhhh too bad, game over, game over! \n{}, you live in a dirty beat-up /nan by the river, wretched and alone!".format(name))
    # call again function and pass in our variables
    again(nice,mean,name)
