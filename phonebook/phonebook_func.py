...
def center_window(self, w, h): # pass in the tkinter frame (master) reference snd the w and h
    # get user's screen width and height
    screen_width = self.master.winfo_screenwidth()
    screen_height = self.master.winfo_screenheight()
    # calculate x and y coordinates to paint the app centered on the user's screen
    x = int((scrren_width/2) - (w/2))
...
def ask_quit(self):
    if messagebox.askokcancancel("Exit program" "Okay to exit application?"):
        # This closes app
        self.master.destroy()
        os._exit(0)
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email FROM tbl_phonebook WHERE col_fullnsme = (?)""", [value])
